...
export default function MechanicsPage() {
  return <Box mt={2} mx={5} mb={30}>
    ...
    <Typography>
      The game is played in individual rounds. Each round, a <i>question</i> is selected, which is a line from
      a Taylor Swift song. The answer to this question will be the next line, and if the player knows the answer,
      they may simply type it in. If they need help in determining the answer, they can use three different types of
      lifelines (which are a scarce resource), and/or reduce the question to a multiple choice question.
      The game continues indefinitely until the player guesses incorrectly, at which point the game ends
      and the final score is determined.
    </Typography>
    ...
    <List sx={{ listStyleType: 'disc', listStylePosition:'inside' }}>
      ...
      <ListItem sx={{ display: 'list-item' }}>
        ...
        If there are not enough previous lines because the question's line occurs too early in the song,
        the ShowPrevLines lifeline show everything from the song's first line, and also indicate
        that the lines shown are the initial lines of the song.
      </ListItem>
      <ListItem sx={{ display: 'list-item' }}>
        The Skip lifeline skips the current question, showing the answer and earning 0 points for the player.
      </ListItem>
    </List>
    ...
    <Typography>
      ...
      score. This distance is calculated case insensitively, so "a" and "A" are treated as the same
      character. The game will also check if truncating the player's guess by any amount would give a smaller
      edit distance, and choose to truncate the guess by the best possible amount. The motivation here
      is that if a player thinks the next line is "You're not sorry, No, no, oh-oh" and the next line was
      actually "You're not sorry" then they should still receive full credit.
    </Typography>
    <Typography>
      ...
      If <i>d</i> = 0 then the player earns 26 points for a perfect match, as well as a new random lifeline.
      For other values of <i>d</i>, The player receives a score of (14 - <i>d</i>) and has a probability
      (1 - <i>d</i>/13) of receiving a new random lifeline.
    </Typography>
    <Typography>
      There is one other possibility: the game might ask the player for a longer guess if the player's
      guess is more than 5 characters shorter than the next line, yet the player's guess is "on the right track".
      A guess is "on the right track" if you can take a prefix of the correct answer such that the edit
      distance between that prefix and the guess is at most 13.
    </Typography>
  </Box>;
}